...
class ProgressMeter(object):
    ...
    def get_meter(self, **kw):
        ...
        if self.rate_current == 0:
            ...
            remEstNum = float((self.total - self.count))/self.rate_current
            remEst = time.strftime("H:%H M:%M S:%S",time.gmtime(remEstNum))
        return '[%s>%s] %d%%  %s remaining' % (bar, pad, perc, remEst)
